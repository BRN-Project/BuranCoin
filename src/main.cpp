...
#if defined(NDEBUG)
#error "BRN cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight, CAmount nFees, bool fBudgetBlock)
{
    ...
    if (nHeight == 1)
        nSubsidy = CAmount(1000000) * COIN; //premine has no budget allocation
    ...
    else if (nHeight < 218880)
        nSubsidy = 6 * nBudgetMultiplier;
    else if (nHeight < 348480)
